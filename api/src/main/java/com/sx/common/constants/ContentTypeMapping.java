package com.sx.common.constants;
...
public class ContentTypeMapping {
     ...
     * 获取文件类型
     *
     * @param fileNameOrType 文件名或文件类型
     * @return 文件类型
     */
    public static String getContentType(String fileNameOrType) {
        String ext = fileNameOrType;
        if (fileNameOrType == null || fileNameOrType.isEmpty()) {
            return MediaType.APPLICATION_OCTET_STREAM_VALUE;
        }
        int lastDotIndex = fileNameOrType.lastIndexOf('.');
        if (lastDotIndex == -1 || lastDotIndex == fileNameOrType.length() - 1) {
            return MediaType.APPLICATION_OCTET_STREAM_VALUE;
        }
        if(lastDotIndex > 0){
            ext = fileNameOrType.substring(lastDotIndex + 1).toLowerCase();
        }
        return Optional.ofNullable(MAP.get(ext)).orElse(MediaType.APPLICATION_OCTET_STREAM_VALUE);
    }
